...
"Counterfactual prediction model performance," *Statistics in Medicine*, 
...
prob_treat <- plogis(-0.5 + 0.5 * x1 + 0.3 * x2)
treatment <- rbinom(n, 1, prob_treat)

# Outcome depends on covariates and treatment
prob_outcome <- plogis(-1 + 0.8 * x1 + 0.5 * x2 - 0.5 * treatment)
outcome <- rbinom(n, 1, prob_outcome)

# Create a prediction model (trained on observed data)
data <- data.frame(x1 = x1, x2 = x2, treatment = treatment, outcome = outcome)
pred_model <- glm(outcome ~ x1 + x2, data = data, family = binomial)
predictions <- predict(pred_model, type = "response")
```
...
For convenience, you can also use a formula interface:
